...
export class EmployeeRepository {
  ...
  public static async findAll(): Promise<Employee[]> {
    return new Promise((resolve, reject) => {
      connection.query('SELECT employee_id, role_id, full_name FROM employee', (error: any, results) => {
        if (error) {
          reject(error);
        } else {
  ...
  public static async createEmployee(employee: Employee): Promise<Employee> {
    const query = 'INSERT INTO employee (full_name, role_id, password, created_at, created_by, updated_at, updated_by, deleted) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
    console.log(employee);
    return new Promise((resolve, reject) => {
      connection.execute(query, [employee.full_name, employee.role_id, employee.password, employee.created_at, employee.created_by, employee.updated_at, employee.updated_by, employee.deleted], (error, result: ResultSetHeader) => {
        if (error) {
          reject(error);
        } else {
  ...
  public static async updateEmployee(employee_id: number, employeeData: Employee): Promise<Employee | null> {
    const query = 'UPDATE employee SET full_name = ?, role_id = ?, password = ?, updated_at = ?, updated_by = ?, deleted = ? WHERE employee_id = ?';
    return new Promise((resolve, reject) => {
      connection.execute(query, [employeeData.full_name, employeeData.role_id, employeeData.password, employeeData.updated_at, employeeData.updated_by, employeeData.deleted, employee_id], (error, result: ResultSetHeader) => {
        if (error) {
          reject(error);
        } else {
